...
return {
  ...
  apply(ctx) {
    ...
    try {
      ...
      if (initiator !== undefined && typeof initiator.id === 'string') defaultSessionId = initiator.id
    } catch (error) { /* agentless activation */ }
    if (defaultSessionId === null) {
      try {
        const roots = agents.roots()
        const any = roots.length > 0 ? roots[0] : agents.list()[0]
        if (any !== undefined && typeof any.id === 'string') defaultSessionId = any.id
      } catch (error) { /* ignore */ }
    }
    ...
    function describeSession(agent) {
    ...
    function statePayload() {
      let sessions = []
      try { sessions = agents.list().map(describeSession) } catch (error) { /* empty */ }
      return {
        ok: true,
        server: { host: ctx.webServer.host, port: ctx.webServer.port },
    ...
    const apiHandler = async (req, res) => {
      ...
      const given = tokenOf(req, params)
    ...
    ctx.effect(() => ctx.webServer.register({ kind: 'exact', path: ROOT, handler: pageHandler }), 'remote-control page route')
    ctx.effect(() => ctx.webServer.register({ kind: 'exact', path: ROOT + '/', handler: pageHandler }), 'remote-control page route (trailing slash)')
    ctx.effect(() => ctx.webServer.register({ kind: 'prefix', path: API_ROOT, handler: apiHandler }), 'remote-control api route')

    ctx.effect(() => ctx.webServer.tapIndex((page) => {
    ...
    harness.handle('rc-info', async () => ({
      ok: true,
      token,
      host: ctx.webServer.host,
      port: ctx.webServer.port,
      defaultSessionId,
      sessions: (() => { try { return agents.list().length } catch (error) { return 0 } })(),
      bridge: { status: bridgeRef.status, port: bridgeRef.port },
      tunnel: { status: tunnelRef.status, url: tunnelRef.url },
    }))

    harness.handle('rc-tunnel', async (args) => {
      const action = args !== null && typeof args === 'object' && args.action === 'stop' ? 'stop' : 'start'
      if (action === 'stop') stopTunnel()
      else await startTunnel()
      return { ok: true, tunnel: { status: tunnelRef.status, url: tunnelRef.url } }
    })

    console.log('[remote-control] 本机网页版入口: http://127.0.0.1:' + ctx.webServer.port + '/?token=' + token)
    console.log('[remote-control] 本机控制页: http://127.0.0.1:' + ctx.webServer.port + ROOT + '/?token=' + token)

    if (ctx.webServer.host === '127.0.0.1' && subprocess !== undefined) {
      ...
      subprocess.resolveExecutable('node').then((nodePath) => {
        ...
        if (typeof nodePath !== 'string' || nodePath.length === 0) {
          ...
          console.log('[remote-control] 未找到 node，无法启动网关（本机访问不受影响）')
          return
        }
        const script = gatewayScript(ctx.webServer.port, BRIDGE_PORT, token)
        let handle
        try {
          ...
          console.log('[remote-control] 网关启动失败: ' + String(error))
          return
        }
        ...
        console.log('[remote-control] 局域网网页版: http://<本机局域网IP>:' + BRIDGE_PORT + '/?token=' + token)
        console.log('[remote-control] 若 Windows 防火墙弹出提示，请允许 node 的网络访问')
    ...
    async function provisionTunnelClient() {
      if (subprocess === undefined) return false
      let nodePath = null
    ...
    async function startTunnel(allowProvision) {
      ...
      if (exe === null || tunnelRef.handle === undefined) {
        if (allowProvision === false) {
          ...
          console.log('[remote-control] 未找到可用的隧道客户端（cloudflared / ngrok）')
          return
        }
        tunnelRef.status = 'provisioning'
        console.log('[remote-control] 未找到隧道客户端，正在自动下载 cloudflared…')
        provisionTunnelClient().then((ok) => {
          if (tunnelRef.disposed) return
          if (!ok) {
            tunnelRef.status = 'failed'
            console.log('[remote-control] 自动下载失败。可手动安装（winget install Cloudflare.cloudflared），然后重新点击“启动公网隧道”。')
            return
          }
      ...
      const scan = () => {
        ...
        for (const pattern of TUNNEL_URL_PATTERNS) {
          ...
          if (match !== null && match.length > 0 && typeof match[0] === 'string') {
            ...
            console.log('[remote-control] 公网网页版入口: ' + match[0] + '/?token=' + token)
      ...
      handle.done.then((outcome) => {
        ...
        if (tunnelRef.status !== 'running') {
          tunnelRef.status = 'failed'
          console.log('[remote-control] 公网隧道进程退出（exitCode=' + (outcome === null ? '?' : String(outcome.exitCode)) + '），请检查网络或客户端配置')
        } else {
          tunnelRef.status = 'none'
          tunnelRef.url = null
        }
    ...
    function gatewayScript(dsPort, gatePort, gateToken) {
      ...
      const loginHtml = [
        ...
        '<div class="box">',
        '<h1>DSH 网页版 · 远程登录</h1>',
        '<p>输入访问令牌（见电脑上 DSH 的 Run 卡片面板）即可使用完整网页版。令牌即密码，请勿泄露。</p>',
